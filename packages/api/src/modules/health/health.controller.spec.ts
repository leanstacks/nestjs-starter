...
describe('HealthController', () => {
  ...
  describe('checkHealth', () => {
    it('should perform health checks and return results when all checks pass', async () => {
      ...
      (typeOrmHealthIndicator.pingCheck as Mock).mockResolvedValue(databaseHealthResult);
      (versionHealthIndicator.getValue as Mock).mockReturnValue(versionHealthResult);
      (healthCheckService.check as Mock).mockResolvedValue(expectedResult);
    ...
    it('should call TypeOrmHealthIndicator.pingCheck with correct parameters', async () => {
      ...
      (typeOrmHealthIndicator.pingCheck as Mock).mockResolvedValue({ database: { status: 'up' } });
      (versionHealthIndicator.getValue as Mock).mockReturnValue({ version: { status: 'up', value: '0.1.0' } });
      (healthCheckService.check as Mock).mockResolvedValue(expectedResult);
      ...
      const checkCalls = (healthCheckService.check as Mock).mock
    ...
    it('should call VersionHealthIndicator.getVersion with correct parameters', async () => {
      ...
      const checkCalls = (healthCheckService.check as Mock).mock.calls[0][0] as (() => HealthIndicatorResult)[];
      checkCalls[1]();
    ...
    it('should handle health check failures gracefully', async () => {
      ...
      (typeOrmHealthIndicator.pingCheck as Mock).mockRejectedValue(new Error('Connection failed'));
      (versionHealthIndicator.getValue as Mock).mockReturnValue({
        version: { status: 'down', error: 'Version not found or not valid semver' },
      });
      (healthCheckService.check as Mock).mockResolvedValue(expectedResult);
    ...
    it('should pass the correct number of health check functions', async () => {
      ...
      // Assert
      const checkCalls = (healthCheckService.check as Mock).mock.calls[0][0] as (() => unknown)[];
    ...
    it('should return health check service result directly', async () => {
      ...
      (typeOrmHealthIndicator.pingCheck as Mock).mockResolvedValue({ database: { status: 'up' } });
      (versionHealthIndicator.getValue as Mock).mockReturnValue({ version: { status: 'up', value: '0.1.0' } });
      (healthCheckService.check as Mock).mockResolvedValue(mockHealthResult);
